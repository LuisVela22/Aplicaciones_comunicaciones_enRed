...
public class Server extends Thread {
    ...
    public void run() {
        ...
        try {
            group = InetAddress.getByName("231.10.2.1");	
        } catch (UnknownHostException e) {
            e.printStackTrace();
            System.exit(1);
        }

        System.out.println("Servidor corriendo en la dirección 231.10.2.1, puerto 4567, esperando clientes...");
        for (;;) {
            try {
                MulticastSocket socket = new MulticastSocket(4567);
                ...
                if(msg.contains("<inicio>")){
                    ...
                    DatagramPacket packet = new DatagramPacket( cont.getBytes(), cont.length(), group, 4567);
                    socket.send(packet);
                    socket.close();
                }else if(msg.contains("C<msj>")){
                    msg = msg.substring(1);
                    msg = "S" + msg;
                    DatagramPacket packet = new DatagramPacket(msg.getBytes(), msg.length(), group, 4567);
                    socket.send(packet);
                    socket.close();
